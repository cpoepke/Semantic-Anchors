...
require_arg() {
  # Validates that a value was provided for an option and is not another flag.
  # Uses an inline case statement (POSIX-valid): returns true when $2 starts with '-',
  # which means no real value was supplied.
...
EOF
  ...
  if grep -qxF "$MARKER_START" "$target_file"; then
    if ! grep -qxF "$MARKER_END" "$target_file"; then
      echo "ERROR: Found $MARKER_START but missing $MARKER_END in $target_file" >&2
      echo "Refusing to replace — would truncate the file. Add the end marker and retry." >&2
      exit 1
    fi
    # Validate exactly one marker pair and correct order (start before end)
    local start_count end_count start_line end_line
    start_count="$(grep -cxF "$MARKER_START" "$target_file")"
    end_count="$(grep -cxF "$MARKER_END" "$target_file")"
    if [ "$start_count" -ne 1 ] || [ "$end_count" -ne 1 ]; then
      echo "ERROR: Expected exactly one start and one end marker in $target_file (found $start_count start, $end_count end)" >&2
      exit 1
    fi
    start_line="$(grep -nxF "$MARKER_START" "$target_file" | head -1 | cut -d: -f1)"
    end_line="$(grep -nxF "$MARKER_END" "$target_file" | head -1 | cut -d: -f1)"
    if [ "$end_line" -le "$start_line" ]; then
      echo "ERROR: End marker appears before start marker in $target_file (line $end_line vs $start_line)" >&2
      exit 1
    fi
    awk -v start="$MARKER_START" -v end="$MARKER_END" -v repl="$INJECTED_FILE" '
      ...
      $0 == start { if (!in_block) printf "%s", replacement; in_block = 1; next }
